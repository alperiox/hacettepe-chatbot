...
@app.command()
def crawl(
    base_url: str, max_depth: int = 1, auto_save: int = 100, save_logs: bool = True
):
...
def auto(base_url: str, max_depth: int = 1, auto_save: int = 100, load: bool = True):
    if load:
        assert os.path.exists(
            "dataset.json"
        ), "No dataset.json found, please run the default crawler first to save an initial dataset"
        crawler = Crawler()
        crawler.load_json()
